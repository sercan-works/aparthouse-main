...
const ClientAparts = ({ 
  ...
  // İlk yükleme sırasında başlangıç değerlerini Redux store'a ekleyin
  useEffect(() => {
  ...
  // filterApi.ts'te tanımlandığı gibi boş string parametresi kullanıyoruz
  ...
  // Client tarafında filtrelenen veriyi getir (SSR ile gelen verilerden sonra güncellemeleri almak için)
  const { data: apiAparts, error, isLoading } = useGetApartsQuery(queryParams);
  
  // Yükleme durumunu kontrol et, ama SSR'den gelen veri varsa göster
  if (isLoading && !initialAparts.length) {
    return (
  ...
  // API'den gelen veriyi kullan, hata varsa SSR'den gelen veriyi göster
  const apartments = error ? initialAparts : (apiAparts || initialAparts);
    ...
    <div className="flex flex-col justify-center items-center">
      {error && (
        <div className="w-full bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-4 mb-5">
          ...
          <p>API bağlantısı sırasında bir hata oluştu. SSR ile gelen veriler gösteriliyor.</p>
